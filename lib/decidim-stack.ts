...
export class DecidimStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: DecidimStackProps) {
    ...
    backendTaskRole.addManagedPolicy(
    ...
    const DecidimContainerEnvironment: { [key: string]: string } = {
      ...
      RDS_USERNAME: ssm.StringParameter.valueForTypedStringParameterV2(
        this,
        `/decidim-cfj/${props.stage}/RDS_USERNAME`
      ),
      RDS_PASSWORD: ssm.StringParameter.valueForTypedStringParameterV2(
        this,
        `/decidim-cfj/${props.stage}/RDS_PASSWORD`
      ),
      SECRET_KEY_BASE: ssm.StringParameter.valueForTypedStringParameterV2(
        this,
    ...
    const container = taskDefinition.addContainer('nginxContainer', {
      ...
      healthCheck: {
        command: ['CMD-SHELL', `curl --fail -s http://localhost || exit 1`],
        retries: 3,
    ...
    sidekiqTaskDefinition.addContainer('sidekiqContainer', {
      ...
      healthCheck: {
        command: ['CMD-SHELL', `ps aux | grep '[s]idekiq'`],
        retries: 3,
        startPeriod: Duration.minutes(2),
        interval: Duration.minutes(1),
      },
    ...
    props.ecs.certificates.forEach((certificate, i) => {
      certificates.push(
        aws_certificatemanager.Certificate.fromCertificateArn(this, `Certificate${i}`, certificate)
      );
    });
