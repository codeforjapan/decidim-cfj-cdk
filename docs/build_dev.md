...
```console
$ cdk --context stage=dev --context tag=${IMAGE_TAG} --profile decidim bootstrap
```
...
```console
$ cdk --context stage=dev --context tag=${IMAGE_TAG} --profile decidim diff  
```

## 6-3. デプロイ実行

```console
$ cdk --context stage=dev --context tag=${IMAGE_TAG} --profile decidim deploy --all  --require-approval never
```
